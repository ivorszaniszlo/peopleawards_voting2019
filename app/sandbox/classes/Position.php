...
class Position
{
    public function INSERT_POS($org, $pos) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sajnáljuk, de a megadott kategória már szerepel az adatbázisban.</div>";
        } else {
            ...
            if($stmt->execute()) {
                echo "<div class='alert alert-success'>Új kategória hozzáadva.</div>";
            }
            $stmt->free_result();
        }
        return $stmt;
    ...
    public function UPDATE_POS($org, $pos, $pos_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Kategóriák sikeresen frissítve.<a href='http://localhost/peopleawards_voting2019/app/sandbox/add_pos.php'><span class='glyphicon glyphicon-backward'></span></a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_POS($pos_id) {
        ...
        if($stmt->execute()) {
            header("location: http://localhost/peopleawards_voting2019/app/sandbox/add_pos.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
